...
Скрипт `import-rates` запускается ежедневно и загружает курс на текущий день.
Добавьте задачу в cron на сервере (`crontab -e`):

```cron
# Обновление курсов валют каждый день в 12:00 МСК
0 9 * * * docker exec finforme-app-1 ./import-rates >> /var/log/import-rates.log 2>&1
```
